...
elsif (defined($calc_sink_minset_option)) # dorectory that contains
{
	my @tracefiles;
	...
	for(readdir DIR) { push(@tracefiles, $calc_sink_minset_option."/".$_) if /^\d{1,3}\.info$/; }
	...
	foreach my $this_tracefile_path (@tracefiles) 
	{
		...
		my $num_sinks_added;
		if(defined($limit_to_file_option))
		{
			$num_sinks_added = tracefile_adds_sinks($sumtotal_tracefile, $this_tracefile_path, $limit_to_file_option);
		}
		else # otherwise, consider all src code files in the tracefile for analysis (undef indicates this)
		{
			$num_sinks_added = tracefile_adds_sinks($sumtotal_tracefile, $this_tracefile_path, undef);
		}
		
		# does this iteration's tracefile add sinks to the minset?
		# the first tracefile is the template for the $sumtotal_tracefile, so it is part of the minset too
		if( ($num_sinks_added > 0) || ($num_processed == 0) )
		{
		...
		print "tracefile $this_tracefile_path: adds $num_sinks_added sinks.\n";
...
sub calc_minset ($$)
{
	...
	foreach my $this_tracefile_path (@tracefiles) 
	{
		...
		if(defined($limit_to_file_option))
		{
			$num_sinks_added = &$analysis_callback($sumtotal_tracefile, $this_tracefile_path, $limit_to_file_option);
		}
		else # otherwise, consider all src code files in the tracefile for analysis (undef indicates this)
		{
			$num_sinks_added = &$analysis_callback($sumtotal_tracefile, $this_tracefile_path, undef);
		}
