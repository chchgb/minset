...
# tracefile_adds_sinks(total_tracefile_path, new_tracefile_path)
...
our @add_count_files; #  corresponds to --add arguments
our @sub_count_files;  # corresponds to --subtract arguments
...
our $calc_minset; # corresponds to --calc-minset argument (directory)
...
if (!GetOptions("directory|d|di=s" => \@directory,
		...
		"extract=s" => \$extract,
		"add=s{2}" => \@add_count_files,
		"subtract=s{2}" => \@sub_count_files,
...
if (@ARGV && !($extract || $remove || $diff || @add_count_files || @sub_count_files))
{
...
elsif (defined($sinks_hit_option))
{
	...
	foreach my $file (keys(%{$result}))
	{
		...
		foreach my $line_num (keys( %{ $result->{$file} }) )
		{
			...
			$hits++ if ( defined($result->{$file}{$line_num}) && ($result->{$file}{$line_num} > 0) );
		}
	}
	print "Sinks hit: $hits\n";
	exit(0);
}
...
elsif (@compare_total or @compare_src_file or @compare_func)
{
	...
	{	#FIXME: change this so it keeps track of which .info files comprise the total_tracefile
		...
		print "Comparison of sinks in .info files specified:\n";
		print "Total tracefile: $info1\nNew tracefile: $info2\n\n";
		
		print "Result of tracefile_adds_sinks(): ";
		print tracefile_adds_sinks($info1, $info2);
...
elsif (@add_count_files)
{
	print "Adding tracefiles...\n";
	# we need $data->{$add_path}->{"sum"} for each file in @add_count_files
	# so, we need use the function get_sumcount($add_path) that returns \%sumcount that we can pass 
	# directly as an argument to add_counts_mod and subtract_counts_mod.
	my $add_path1 = $add_count_files[0];
	my $add_path2 = $add_count_files[1];

	# $infofile_exhX is the %data hash returned from read_info_file
	my $infofile1 = read_info_file($add_path1);
	my $infofile2 = read_info_file($add_path2);
...
Operation:
 ...
 --add FILE FILE		                Add two tracefiles
 --subtract FILE BASE	                Subtract FILE from BASE (where base is the baseline)
 --compare-total FILE FILE              Compare total coverage of two info files.
 --compare-src-file SRC_CODE FILE FILE  Compare coverage of a specified src code file
                                        that is present in two .info files.
 --compare-func FUNC_NAME FILE FILE     Compare coverage of a specified function that
                                        is present in two .info files.
...
sub check_options()
{
	...
	if( (!$output_filename) && (@sub_count_files || @add_count_files) )
	{
...
# this file.
#
#FIXME revert back to original error handling of die()ing on fail
...
sub get_rat_report($)
{
	...
	foreach $rat_line (@rat_data)
	{
		...
		if ($rat_line =~ m|(.*):(\d+):\s*(\w+):\s*(.*)| )
		{
			...
			print_debug("filename $filename - Line number: ".$ret_hash{$filename}[$count-2]."\n");
