...
# NOTE: when using add_count_files or sub_count_files, each element (each infofile) must have been
#            generated from identical code.
our @add_count_files;  # Files to "add" execution counts for, per line
our @sub_count_files;   # File to "subtract" execution counts for, per line
...
if (!GetOptions("output-directory=s"	=> \$output_directory,
		...
		"prefix=s"		=> \$dir_prefix,
		"add=s"                    => \@add_count_files,
		"subtract=s"           => \@sub_count_files,
...
may also be a list of filenames.
  ...
  -p, --prefix PREFIX               Remove PREFIX from all directory names
      ...
      --add INFOFILE(S)              Add per line execution counts for multiple datasets (infofiles)
      --subtract INFOFILE(S)              Subtract per line execution counts for multiple datasets (files)
NOTE: when using --add or --subtract, each element (each infofile) must have been generated from identical code.
...
sub gen_html()
{
	...
	# Before we start normal execution path, check if see if differential coverage options
	# were used (--add, --subtract). If so, divert the flow to the appropriate wrappers.
...
# 	In abstract terms, this function "adds" data1_ref + data2_ref, which = data_total_ref (the return value).
#
# data1_ref, data2_ref, and data_total_ref (above) are references to hashes containing a mapping
#   line number -> execution count
#
# Return a reference to a hash containing the same mapping as above:
#    line number-> total execution count
#
...
sub add_counts_mod($$)
{
	...
	foreach $line (keys(%data1))
	{
		...
		{
			# this condition should not happen, we should be working with identical code, error condition?
			print "Warning: only use add_counts_mod() with %data1 and %data2 having identical line numbers (same code)\n"
			# TODO exit? Handle this warning properly, or remove entirely.
		}
...
# 	In abstract terms, this function "subtracts" data_ref - base_ref, which = data_diff_ref (the return value).
#
# data_ref, base_ref, and data_diff_ref (above) are references to hashes containing a mapping
#   line number -> execution count
#
# Return a reference to a hash containing the same mapping as above:
#    line number-> execution count difference
#
...
sub subtract_counts_mod($$)
{
	...
	foreach $line (keys(%data))
	{
		...
		{
			# this condition should not happen, we should be working with identical code, error condition?
			print "Warning: only use subtract_counts_mod() with %data and %base having identical line numbers (same code)\n"
			# TODO exit? Handle this warning properly, or remove entirely.
		}
